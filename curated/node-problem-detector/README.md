...
```console
$ helm install <helm-repo>/node-problem-detector
```
...
```console
$ helm install --name my-release <helm-repo>/node-problem-detector
```
...
```console
Specify each parameter using the `--set key=value[,key=value]` argument to `helm install` or provide a YAML file containing the values for the above parameters:
$ helm install --name my-release <helm-repo>/node-problem-detector --values values.yaml
```
