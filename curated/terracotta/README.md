...
```bash
$ helm install <helm-repo>/terracotta
```
...
```bash
$ helm install --name my-release <helm-repo>/terracotta
```
...
helm install --dry-run --name my-release  --set replicaCount=4   --debug <helm-repo>/terracotta
Specify each parameter using the `--set key=value[,key=value]` argument to `helm install`. For example,

```bash
$ helm install --name my-release \
  --set replicaCount=3  --set offheaps[0].name=otherOffheap --set offheaps[0].unit=GB --set offheaps[0].size=6 \
    <helm-repo>/terracotta
$ helm install --name my-release \
```
...
```bash
$ helm install --name my-release -f values.yaml <helm-repo>/terracotta
```
