...
```bash
$ helm install --name my-release <helm-repo>/spinnaker --timeout 600
```
...
```bash
$ helm install --name my-release -f values.yaml <helm-repo>/spinnaker
```
...
```shell
$ helm install -n cd <helm-repo>/spinnaker
