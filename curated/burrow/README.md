...
```bash
helm install <helm-repo>/burrow --name my-release --values initialize.yaml
```
...
```bash
Specify each parameter using the `--set key=value[,key=value]` argument to `helm install`. For example,
helm install <helm-repo>/burrow --name my-release \
...
```bash
helm install <helm-repo>/burrow --name my-release -f initialize.yaml
```
