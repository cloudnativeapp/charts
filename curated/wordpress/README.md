...
```console
$ helm install --name my-release <helm-repo>/wordpress
```
...
```console
$ helm install --name my-release -f values.yaml <helm-repo>/wordpress
```
...
```console
$ helm install --name my-release -f ./values-production.yaml <helm-repo>/wordpress
```
...
```console
$ helm install <helm-repo>/nfs-server-provisioner --set persistence.enabled=true,persistence.size=10Gi
$ helm install --name my-release -f values-production.yaml --set persistence.storageClass=nfs <helm-repo>/wordpress --set mariadb.master.persistence.storageClass=nfs
```
...
```
helm install <helm-repo>/wordpress --set allowOverrideNone=yes
```
...
```
helm install <helm-repo>/wordpress --set allowOverrideNone=yes --set customHTAccessCM=custom-htaccess
```
