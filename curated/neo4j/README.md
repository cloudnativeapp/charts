...
```bash
$ helm install --name neo4j-helm <helm-repo>/neo4j --set acceptLicenseAgreement=yes --set neo4jPassword=mySecretPassword
```
...
```bash
$ helm install --name neo4j-helm --set core.numberOfServers=5,readReplica.numberOfServers=3 <helm-repo>/neo4j
```
...
```bash
$ helm install --name neo4j-helm -f values.yaml <helm-repo>/neo4j
```
