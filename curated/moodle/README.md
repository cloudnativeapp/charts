...
```console
$ helm install --name my-release <helm-repo>/moodle
```
...
```console
$ helm install --name my-release -f values.yaml <helm-repo>/moodle
```
...
```console
$ helm install --name my-release \
  --set ingress.enabled=True,ingress.hosts[0]=moodle.domain.com,serviceType=ClusterIP,moodleUsername=admin,moodlePassword=password,mariadb.mariadbRootPassword=secretpassword <helm-repo>/moodle
```
$ helm install --name my-release \
