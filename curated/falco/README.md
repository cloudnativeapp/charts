...
```bash
$ helm install --name my-release <helm-repo>/falco
```
...
```bash
Specify each parameter using the `--set key=value[,key=value]` argument to `helm install`. For example,
$ helm install --name my-release --set falco.jsonOutput=true <helm-repo>/falco
```
...
```bash
$ helm install --name my-release -f values.yaml <helm-repo>/falco
```
...
```bash
$ helm install --name falco -f custom-rules.yaml <helm-repo>/falco
```
