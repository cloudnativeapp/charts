...
```console
$ helm install <helm-repo>/cosbench
```
...
```console
$ helm install --name my-release <helm-repo>/cosbench
```
...
```console
Specify each parameter using the `--set key=value[,key=value]` argument to `helm install`. For example,
$ helm install <helm-repo>/cosbench --name my-release \
...
```console
$ helm install <helm-repo>/cosbench --name my-release -f values.yaml
```
