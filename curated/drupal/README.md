...
```console
$ helm install --name my-release <helm-repo>/drupal
```
...
```console
$ helm install --name my-release -f values.yaml <helm-repo>/drupal
```
...
```bash
$ helm install --name my-release --set persistence.drupal.existingClaim=PVC_NAME <helm-repo>/drupal
```
...
1. Install the chart
    ...
    ```bash
    $ helm install --name my-release --set persistence.drupal.hostPath=/PATH/TO/HOST/MOUNT <helm-repo>/drupal
    ```
