...
```bash
$ helm install <helm-repo>/zetcd --name my-release
```
...
```bash
Specify each parameter using the `--set key=value[,key=value]` argument to `helm install`. For example:
$ helm install --name my-release --set image.tag=v0.0.3 <helm-repo>/zetcd
```
...
```bash
$ helm install --name my-release --values values.yaml <helm-repo>/zetcd
```
