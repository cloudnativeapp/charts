...
```bash
$ helm install <helm-repo>/kapacitor --name foo --namespace bar
```
...
```bash
$ helm install --name my-release <helm-repo>/kapacitor
```
...
```bash
$ helm install --name my-release -f values.yaml <helm-repo>/kapacitor
```
