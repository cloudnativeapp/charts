...
```bash
$ helm install --name gocd-app --namespace gocd <helm-repo>/gocd
```
...
```bash
$ helm install --namespace gocd --name gocd-app --set server.shouldPreconfigure=false <helm-repo>/gocd
```
...
Alternatively, a YAML file that specifies the values for the parameters can be provided while installing the chart. For example,
             |
| `server.persistence.storageClass`             | The PVC storage class nam

```bash
$ helm install --namespace gocd --name gocd-app -f values.yaml <helm-repo>/gocd
```
...
```bash
$ helm install --namespace gocd --name gocd-app --set server.persistence.storageClass=STORAGE_CLASS_NAME <helm-repo>/gocd
```
...
```
$ helm install --name gocd-app --set server.persistence.existingClaim=PVC_NAME <helm-repo>/gocd
```
...
```bash
helm install --namespace gocd --name gocd-app --set rbac.roleRef=ROLE_NAME <helm-repo>/gocd
```
