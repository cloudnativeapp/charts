...
```bash
$ helm install <helm-repo>/etcd-operator --name my-release
```
...
```bash
Specify each parameter using the `--set key=value[,key=value]` argument to `helm install`. For example:
$ helm install --name my-release --set image.tag=v0.2.1 <helm-repo>/etcd-operator
```
...
```bash
$ helm install --name my-release --values values.yaml <helm-repo>/etcd-operator
```
...
```console
$ helm install --name my-release <helm-repo>/etcd-operator --set rbac.create=false
```
...
```console
$ helm install --name my-release <helm-repo>/etcd-operator --set rbac.install=true,rbac.apiVersion=v1alpha1
```
