...
```bash
$ helm install <helm-repo>/influxdb --name foo --namespace bar
```
...
```bash
$ helm install --name my-release <helm-repo>/influxdb
```
...
```bash
$ helm install --name my-release -f values.yaml <helm-repo>/influxdb
```
