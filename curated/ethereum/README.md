...
```console
$ helm install <helm-repo>/ethereum
```
...
2. Install the chart as follows:
    ...
    ```console
    $ helm install --name my-release <helm-repo>/ethereum
...
```console
Specify each parameter using the `--set key=value[,key=value]` argument to `helm install`. For example, to configure the networkid:
$ helm install <helm-repo>/ethereum --name ethereum --set geth.genesis.networkid=98052
```
...
```console
$ helm install <helm-repo>/ethereum -f values.yaml
```
