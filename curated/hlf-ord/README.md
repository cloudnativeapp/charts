...
```bash
$ helm install <helm-repo>/hlf-ord
```
...
```bash
$ helm install <helm-repo>/hlf-ord --name ord1
```
...
```bash
Specify each parameter using the `--set key=value[,key=value]` argument to `helm install`. For example:
$ helm install <helm-repo>/hlf-ord --name ord1 --set ord.mspID=MyMSP
```
...
```bash
$ helm install <helm-repo>/hlf-ord --name ord1 -f my-values.yaml
```
