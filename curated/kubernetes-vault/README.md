...
```bash
$ helm install --name my-release <helm-repo>/kubernetes-vault --set vault.address=$your_vault_server --set vault.token=$your_vault_token
```
...
```bash
$ helm install --name my-release -f values.yaml <helm-repo>/kubernetes-vault
```
