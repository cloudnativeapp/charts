...
```console
$ helm install <helm-repo>/kanister-operator
```
...
```console
$ helm install --name my-release <helm-repo>/kanister-operator
```
...
```console
You can also specify any non-array parameter using the `--set key=value[,key=value]` argument to `helm install`. For example,
$ helm install <helm-repo>/kanister-operator --name my-release \
