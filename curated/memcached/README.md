...
```console
$ helm install <helm-repo>/memcached
```
...
```console
$ helm install --name my-release <helm-repo>/memcached
```
...
```console
Specify each parameter using the `--set key=value[,key=value]` argument to `helm install`. For example,
$ helm install --name my-release --set memcachedUser=user,memcachedPassword=password <helm-repo>/memcached
```
...
```console
$ helm install --name my-release -f values.yaml <helm-repo>/memcached
```
