...
```console
$ helm install --name my-release <helm-repo>/etcd
```
...
$ helm install --name my-release \
  --set auth.rootPassword=secretpassword <helm-repo>/etcd
```
$ helm install --name my-release \
...
```console
$ helm install --name my-release -f values.yaml <helm-repo>/etcd
```
...
$ helm install <helm-repo>/etcd --set envVarsConfigMap=etcd-env-vars
```
...
$ helm install <helm-repo>/etcd --set configFileConfigMap=etcd-conf
```
...
```console
$ helm install --name my-release -f ./values-production.yaml <helm-repo>/etcd
```
...
```console
$ helm install --name my-release --set auth.rbac.enabled --set auth.rbac.rootPassword=YOUR-PASSWORD <helm-repo>/etcd
...
```console
$ helm install --name my-release --set auth.peer.secureTransport=true --set auth.peer.useAutoTLS=true <helm-repo>/etcd
...
```console
$ helm install --name my-release --set auth.client.secureTransport=true --set auth.client.enableAuthentication=true --set auth.client.existingSecret=etcd-client-certs <helm-repo>/etcd
