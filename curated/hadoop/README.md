...
```
$ helm install --name hadoop $(<helm-repo>/hadoop/tools/calc_resources.sh 50) <helm-repo>/hadoop
```
...
```
$ helm install --name hadoop $(<helm-repo>/hadoop/tools/calc_resources.sh 50) \
...
```
helm install --set hadoop.useConfigMap=true <helm-repo>/zeppelin
```
