...
## TL;DR:
```bash
helm install <helm-repo>/aws-alb-ingress-controller --set clusterName=MyClusterName --set autoDiscoverAwsRegion=true --set autoDiscoverAwsVpcID=true
```
...
```bash
helm install <helm-repo>/aws-alb-ingress-controller --set clusterName=MyClusterName --set autoDiscoverAwsRegion=true --set autoDiscoverAwsVpcID=true --name my-release --namespace kube-system
```
...
```bash
helm install <helm-repo>/aws-alb-ingress-controller --name my-release -f values.yaml
```
