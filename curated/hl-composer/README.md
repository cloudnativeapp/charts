...
```bash
$ helm install <helm-repo>/hl-composer
```
...
```bash
$ helm install <helm-repo>/hl-composer --name hlc
```
...
```bash
Specify each parameter using the `--set key=value[,key=value]` argument to `helm install`. For example:
$ helm install <helm-repo>/hl-composer --name hlc --set persistence.enabled=true,presistence.storageClass=azurefile
```
...
```bash
$ helm install <helm-repo>/hl-composer --name hlc -f my-values.yaml
```
