...
```bash
$ helm install <helm-repo>/chronograf --name foo --namespace bar
```
...
```bash
$ helm install --name my-release <helm-repo>/chronograf
```
...
```bash
$ helm install --name my-release -f values.yaml <helm-repo>/chronograf
```
