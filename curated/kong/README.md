...
```bash
$ helm install --name my-release <helm-repo>/kong
```
...
```console
Specify each parameter using the `--set key=value[,key=value]` argument to `helm install`. For example,
$ helm install <helm-repo>/kong --name my-release \
...
```console
$ helm install <helm-repo>/kong --name my-release -f values.yaml
```
...
```bash
# without a database
helm install <helm-repo>/kong --set ingressController.enabled=true \
...
```shell
helm install <helm-repo>/kong --set ingressController.enabled=true --set proxy.loadBalancerIP=[Your IP goes there] --set proxy.type=LoadBalancer --name kong --namespace kong
```
