...
```bash
$ helm install <helm-repo>/hlf-peer
```
...
```bash
$ helm install <helm-repo>/hlf-peer --name peer1
```
...
```bash
Specify each parameter using the `--set key=value[,key=value]` argument to `helm install`. For example:
$ helm install <helm-repo>/hlf-peer --name peer1 --set peer.mspID=MyMSP
```
...
```bash
$ helm install <helm-repo>/hlf-peer --name peer1 -f my-values.yaml
```
