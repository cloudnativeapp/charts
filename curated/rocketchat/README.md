...
```console
$ helm install <helm-repo>/rocketchat --set mongodb.mongodbPassword=$(echo -n $(openssl rand -base64 32)),mongodb.mongodbRootPassword=$(echo -n $(openssl rand -base64 32))
```
...
```console
$ helm install --name rocketchat <helm-repo>/rocketchat
```
...
```bash
$ helm install --name rocketchat -f values.yaml <helm-repo>/rocketchat
```
