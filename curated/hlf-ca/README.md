...
```bash
$ helm install <helm-repo>/hlf-ca --name org1-ca
```
...
```bash
Specify each parameter using the `--set key=value[,key=value]` argument to `helm install`. For example:
$ helm install <helm-repo>/hlf-ca --name org1-ca --set adminUsername=ca-admin,adminPassword=secretpassword
```
...
```bash
$ helm install <helm-repo>/hlf-ca --name org1-ca -f my-values.yaml
```
...
```
 helm install <helm-repo>/hlf-ca --name org1-ca --set postgresql.enabled=true
```
...
```
 helm install <helm-repo>/hlf-ca --name org1-ca --set mysql.enabled=true
```
