...
```bash
$ helm install --name my-release --set sysdig.accessKey=YOUR-KEY-HERE <helm-repo>/sysdig
```
...
```bash
$ helm install --name my-release -f values.yaml <helm-repo>/sysdig
```
...
```bash
$ helm install --name my-release -f custom-app-checks.yaml -f values.yaml <helm-repo>/sysdig
```
