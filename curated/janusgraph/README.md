...
```shell
helm install --name my-release <helm-repo>/janusgraph
```
...
```shell
helm install --name my-release -f values.yaml <helm-repo>/janusgraph
```
...
```bash
$ helm install --set persistence.existingClaim=<your-persistent-volume-claim> <helm-repo>/janusgraph
```
